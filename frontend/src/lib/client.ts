...
export const api = {
  listMeetings: () => apiFetch<Meeting[]>("/api/meetings"),
  ...
  createTask: (payload: { project_id: number; title: string; description?: string }) =>
    apiFetch<Task>("/api/tasks", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }),
  patchTask: (id: number, payload: Partial<Pick<Task, "title" | "description" | "assignee" | "priority" | "status">>) =>
