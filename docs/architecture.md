...
```
queued → processing → transcribing → extracting → done
...
**Resume semantics:** retry (only from `error`) re-enters the pipeline and skips completed stages — wav exists → skip FFmpeg; segments exist → skip whisper; tasks exist → skip extraction. A failed commit rolls back before the error state is persisted, so a meeting can never get stuck mid-status. On server restart, meetings stranded in active statuses are swept to `error` with a retry hint.
